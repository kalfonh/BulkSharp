...
public static class BulkSharpEntityFrameworkExtensions
{
        ...
        where TContext : BulkSharpDbContext
    {
        ...
        // Overrides the in-memory event store the same way the repositories above override
        // their in-memory counterparts: the defaults are registered with TryAdd, so the last
        // registration wins. Without this a host with durable operation storage would still
        // lose events on restart, and a scaled-out service would hand clients sequence
        // numbers that are not comparable between instances.
