...
internal sealed class BulkStepExecutorService : IBulkStepExecutor
{
    ...
    private readonly IRowRecordPersistenceProvider _persistenceProvider;
    ...
    public BulkStepExecutorService(
        ...
        IRowRecordPersistenceProvider persistenceProvider,
    ...
    public async Task ExecuteStepAsync<TMetadata, TRow>(
        ...
        where TRow : class, IBulkRow, new()
    {
        var persistence = _persistenceProvider.GetPersistence(step);

        var record = BulkRowRecord.CreateStep(operationId, rowNumber, row.RowId, step.Name, stepIndex);

        await persistence.CreateAsync(record, cancellationToken).ConfigureAwait(false);
        ...
        try
        {
            ...
            {
                await HandleAsyncCompletionAsync(asyncStep, row, metadata, record, persistence, cancellationToken).ConfigureAwait(false);
            }
        ...
        catch (BulkStepTimeoutException)
        {
            ...
            await persistence.UpdateAsync(record, cancellationToken).ConfigureAwait(false);
            throw;
        }
    ...
    private async Task HandleAsyncCompletionAsync<TMetadata, TRow>(
        ...
        IRowRecordPersistence persistence,
