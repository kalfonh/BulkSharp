...
internal sealed class BulkOperationService(
    ...
    IBulkRetryService retryService,
    ...
    ILogger<BulkOperationService> logger) : IBulkOperationService
{
    ...
    public Task<RetrySubmission> RetryFailedRowsAsync(Guid operationId, RetryRequest request, CancellationToken cancellationToken = default) =>
        retryService.RetryFailedRowsAsync(operationId, request, cancellationToken);

    public Task<RetrySubmission> RetryRowAsync(Guid operationId, int rowNumber, CancellationToken cancellationToken = default) =>
        retryService.RetryRowAsync(operationId, rowNumber, cancellationToken);

    public Task<RetryEligibility> CanRetryAsync(Guid operationId, CancellationToken cancellationToken = default) =>
        retryService.CanRetryAsync(operationId, cancellationToken);

    public Task<ExportResult> ExportAsync(Guid operationId, ExportRequest request, CancellationToken cancellationToken = default) =>
        exportService.ExportAsync(operationId, request, cancellationToken);

    public Task<PagedResult<BulkRowRetryHistory>> QueryRetryHistoryAsync(BulkRowRetryHistoryQuery query, CancellationToken cancellationToken = default) =>
        retryHistoryRepository.QueryAsync(query, cancellationToken);
}
