...
- **Record lifecycle**: `IBulkStepRecordManager` owns `BulkRowRecord` creation, loading, and state transitions. The step executor delegates all record persistence to the record manager, keeping execution logic separate from storage concerns. This separation enables the retry path to load existing records (via `GetStepRecordAsync`) while the initial path creates new ones (via `CreateStepRecordAsync`) — both using the same executor for processing.
...
- Handlers are **auto-discovered** from scanned assemblies — just implement `IBulkOperationEventHandler` and it's registered automatically. For explicit control, use `builder.AddEventHandler<MyHandler>()`.
