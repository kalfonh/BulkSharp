...
- Snapshots each failed row's error state into `BulkRowRetryHistory` before resetting
- Resets failed rows to `Pending` and re-queues the operation
- Resumes pipeline operations from the failing step (skips completed steps)
...
See [Retry Guide](retry.md) for configuration, API usage, and a full walkthrough.
