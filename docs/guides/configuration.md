...
Register event handlers to receive notifications when operations change state:

```csharp
services.AddBulkSharp(builder => builder
    .AddEventHandler<EmailNotificationHandler>()
    .AddEventHandler<AuditLogHandler>());
```

Handlers implement `IBulkOperationEventHandler` with default interface methods — only override what you need:
