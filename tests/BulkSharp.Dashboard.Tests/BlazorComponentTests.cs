...
public class BlazorComponentTests : TestContext
{
    ...
    {
        // Index makes HTTP calls in OnInitializedAsync: bulks, operations, then operations again for template
    ...
    {
        var httpClient = CreateMockHttpClient(
            (HttpStatusCode.OK, JsonContent.Create(Array.Empty<object>())),  // operations discovery
            (HttpStatusCode.OK, JsonContent.Create(Array.Empty<BulkOperation>()))  // bulks list
        );
        Services.AddSingleton(httpClient);
    ...
    public void Jobs_WithJobs_DisplaysJobsTable()
    {
        ...
        var httpClient = CreateMockHttpClient(
            (HttpStatusCode.OK, JsonContent.Create(Array.Empty<object>())),  // operations discovery
            (HttpStatusCode.OK, JsonContent.Create(pagedResult))  // bulks list
        );
        Services.AddSingleton(httpClient);
